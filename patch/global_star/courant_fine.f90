...
subroutine energy_fine(ilevel)
  ...
  logical,save::compute_volumes=.true.
#ifndef WITHOUTMPI
  integer::info
  ...
  if (compute_volumes.eqv..true.) then
    ...
    ! Compute global quantities
#ifndef WITHOUTMPI
    comm_buffin(1)=vol_heat
    comm_buffin(2)=vol_cool
    call MPI_ALLREDUCE(comm_buffin,comm_buffout,2,MPI_DOUBLE_PRECISION,MPI_SUM,&
       &MPI_COMM_RAMSES,info)
