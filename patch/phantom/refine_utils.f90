     ...
     if(poisson)then
        ...
        if (mond) then
           ...
           do j=1,twotondim
              ...
              do idim=1,ndim
                 do i=1,nn
                    f_newton(iskip+ind_grid_son(i),idim) = f_newton(ind_fathers(i,0),idim)
                    f_mond  (iskip+ind_grid_son(i),idim) = f_mond  (ind_fathers(i,0),idim)
                 end do
              end do
              do i=1,nn
                 phi_newton    (iskip+ind_grid_son(i)) = phi_newton    (ind_fathers(i,0))
                 phi_mond      (iskip+ind_grid_son(i)) = phi_mond      (ind_fathers(i,0))
                 phi_old_newton(iskip+ind_grid_son(i)) = phi_old_newton(ind_fathers(i,0))
                 phi_old_mond  (iskip+ind_grid_son(i)) = phi_old_mond  (ind_fathers(i,0))
              end do
           end do
        else
           ! Scatter to children cells
           ! Original code: Newton only!
           do j=1,twotondim
              iskip=ncoarse+(j-1)*ngridmax
              do idim=1,ndim
                 do i=1,nn
  ...
  do ind=1,twotondim
     ...
     if(poisson)then
        ...
        if (mond) then
           ! Newton + MOND
           do i=1,nn
              rho_newton    (ind_cell_son(i))=0.0D0
              phi_newton    (ind_cell_son(i))=0.0D0
              phi_old_newton(ind_cell_son(i))=0.0D0
              rho_mond      (ind_cell_son(i))=0.0D0
              phi_mond      (ind_cell_son(i))=0.0D0
              phi_old_mond  (ind_cell_son(i))=0.0D0
           end do
           do idim=1,ndim
              do i=1,nn
                 f_newton(ind_cell_son(i),idim)=0.0D0
                 f_mond  (ind_cell_son(i),idim)=0.0D0
              end do
           end do
        else
           ! Newton only
