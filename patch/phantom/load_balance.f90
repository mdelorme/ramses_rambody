...
subroutine defrag
  ...
  !~~~~~~~~~ begin ~~~~~~~~~
  if (mond) then
     ! Repeat the above (everything below "if (poisson)")
     ! for phi=>phi_newton and f=>f_newton
     ! also for phi_mond and f_mond
     ...
     do ilevel=1,nlevelmax
        do ibound=1,nboundary+ncpu
           ...
           if(ncache>0)then
              igrid=istart
              do i=1,ncache
                 hilbert_key(ngrid2+i)=real(phi_mond(iskip2+igrid),kind=qdp)
                 igrid=next(igrid)
              end do
              ngrid2=ngrid2+ncache
           end if
     ...
     do igrid=1,igridmax
        phi_mond(iskip2+igrid)=real(hilbert_key(igrid),kind=8)
     end do
     end do
     ...
     do ilevel=1,nlevelmax
        do ibound=1,nboundary+ncpu
           ...
           if(ncache>0)then
              igrid=istart
              do i=1,ncache
                 hilbert_key(ngrid2+i)=real(f_mond(iskip2+igrid,idim),kind=qdp)
                 igrid=next(igrid)
              end do
              ngrid2=ngrid2+ncache
           end if
     ...
     do igrid=1,igridmax
        f_mond(iskip2+igrid,idim)=real(hilbert_key(igrid),kind=8)
     end do
     end do
     end do
