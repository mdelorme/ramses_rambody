...
subroutine virtual_tree_fine(ilevel)
  ...
  if(restart_init) particle_data_width=particle_data_width+nvar_min
...
subroutine fill_comm(ind_part,ind_com,ind_list,np,ilevel,icpu)
  ...
  if(restart_init) then
     do ivar=1,nvar_min
        do i=1,np
           reception(icpu,ilevel)%up(ind_com(i),current_property)=varp(ind_part(i),ivar)
        end do
        current_property = current_property+1
     end do
  endif
...
subroutine empty_comm(ind_com,np,ilevel,icpu)
  ...
  if(restart_init) then
     do ivar=1,nvar_min
        do i=1,np
           varp(ind_part(i),ivar)=emission(icpu,ilevel)%up(ind_com(i),current_property)
        end do
        current_property = current_property+1
     end do
  endif
